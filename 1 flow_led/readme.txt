...
选择eda：assignment-setting-eda tool setting-simulate选择modelsim
...
手动仿真：
sim-新建tb文件夹（用于存放激励文件）
打开modelsim
新建工程：file-new-project
取名：为什么工程就用什么名字 flow_led_tb
选择路径 放在sim中 其他选项默认 ok即可
选择第二个 add existing file（添加现有文件）
选择browse 选择rtl文件夹 选择.v文件 点击ok即可

创建tb文件，可以选用以前.vt文件，这里手动写
add new file-选择verilog
browse选择sim-tb文件夹 +/flow_led_tb

可设置：tool-edit perference 根据需求设置即可
 
`timescale 1ns/1ns 单位/精度
#10 延时10ns
#10.5 不可，因为精度为1ns 可修改为1ns/1ps即可

注意：inist语句或者always语句：reg
          其他：wire
代码如下：
`timescale 1ns/1ns  
...
-----------------------完-------------------
modelsim 编译文件 ： 右键文件-compiles-compile out-date（编译修改过的文件）

开始仿真：simulate-start simulation